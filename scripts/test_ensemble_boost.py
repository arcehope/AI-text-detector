...
# --- Helper for grammar score ---
# Since dataset is clean essays, grammar perfection is 100% for AI, and varies for human
# Let's approximate grammar score as 100% for this test to measure accuracy
def get_final_score(text, boost_val):
    ...
    # Perfect grammar boost
    final_score = ensemble_score + boost_val
    return min(100.0, final_score)

# Evaluate over the dataset for different boosts
for boost in [0.0, 5.0, 10.0, 15.0]:
    correct = 0
    correct_ai = 0
    correct_human = 0
    ...
    for idx, row in df.iterrows():
        ...
        final_percentage = get_final_score(text, boost)
        # Apply the adjusted threshold:
        # > 60% is AI
        # However, to label as AI vs Human, let's say the final classification threshold is 50%
        pred = 1 if final_percentage > 50 else 0
        if pred == label:
            correct += 1
            if label == 1:
                correct_ai += 1
    ...
    print(f"Grammar Boost: {boost:<4} | Accuracy: {acc:.2f}% | AI Recall: {ai_rec:.2f}% | Human Specificity: {h_spec:.2f}%")
